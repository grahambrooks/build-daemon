...
build_daemon::build_daemon(const char *path, const char *user_cmd)
    : watched_path(path), user_cmd(user_cmd), building(false), builder("make") {
...
int build_daemon::run(int argc, char *argv[]) {
  ...
  const char *cmd = NULL;
  bool trigger_initial_build = false;

  for (auto arg = 1; arg < argc; arg++) {
    if (is::verbose_option(argv[arg])) {
      std::cout << "lazybuild version 0.7" << std::endl;
      return 0;
    } else {
      if (is::trigger_option(argv[arg])) {
        trigger_initial_build = true;
      } else {
        if (path == NULL) {
          path = argv[arg];
        } else {
...
*/
  ...
  return daemon.run(trigger_initial_build);
}


int build_daemon::run(bool trigger_initial_build) {
